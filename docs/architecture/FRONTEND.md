...
| `lib/library/*` | Retrieval subsystem: `types.ts` (roles, assets, intents, provenance), `planner.ts` (intent building), `ranking.ts`, `freesound.ts` / `pixabay.ts` / `userLibrary.ts` (sources), `service.ts` (orchestration), `cache.ts` (IndexedDB), `clipAudio.ts` (audition/decode), `credits.ts` (attribution export). | Generation, mixing |
