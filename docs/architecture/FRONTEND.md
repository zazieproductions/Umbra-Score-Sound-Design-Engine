...
| `lib/library/*` | Retrieval subsystem: `types.ts` (roles, assets, intents, provenance), `planner.ts` (intent building), `ranking.ts`, `freesound.ts` / `pixabay.ts` / `userLibrary.ts` (sources), `freesoundBackend.ts` (safe integration client → `/api/integrations/freesound/*`), `service.ts` (orchestration), `cache.ts` (IndexedDB), `clipAudio.ts` (audition/decode), `credits.ts` (attribution export). | Generation, mixing, any credential storage |
...
useStudio (Project + clips)
   ...
   └─► library/service.ts ──► /api/library/freesound (backend-authenticated) / IndexedDB / CLAP-rerank ──► AudioClip
```
