...
| DEBT-004 | Export | Loudness conformance (-16 LUFS / -1 dBTP) is implemented in `render.ts` but has no automated conformance test on a known signal. | Medium | Medium | Offline-render a synthetic fixture in a test, measure LUFS/peak, assert conformance |
